...
export async function GET(req: NextRequest, { params }: { params: { track_id: string } }) {
  ...
  const r = await fetch(`${API_BASE}/api/v1/similar/track/${encodeURIComponent(params.track_id)}`, {
    headers,
  });
